...
!                 dble(-t0),dble(ttp(irec)),dble(DT),NSTEP,f0(igaus),rf_tshift,maxit,minderr,&
!                 net,sta,chan_dat, bandname, window_chi,adj_r_tw,adj_z_tw)
!     glob_window_chi(irec,2,1:NCHI,1) = window_chi(1:NCHI)
!     misfit = window_chi(15)/avgamp/avgamp *DT
...
!     write(*,*)'stnm,chan,misfit:',trim(net)//'.'//trim(sta),chan_dat, total_misfit
...
subroutine meas_adj_noise(ievt, irec, glob_sem_disp,iband, fstart0, fend0, bandname, &
    ...
    do icomp=1,NRCOMP
       ...
       if ( findfile ) then
          ...
          dt_inp=0.01 !!!! Here, I set dt=0.01 to interpolate data and syn
          t0_inp=-10.
          ...
          if(.not. USE_NEAR_OFFSET)then!BinHe added
             tstart=(GROUPVEL_MAX(iband)+GROUPVEL_MIN(iband))*LONG_P(iband)/2.0
             if(dist<tstart)then
                 dat_inp_bp(:)=1.0e-10
                 syn_inp_bp(:)=1.0e-10
             endif
          ...
          win_b=floor((dist/GROUPVEL_MAX(iband)-LONG_P(iband)/2.+t0_inp)/dt_inp)!floor((dist/5+t0_inp)/dt_inp)
          win_e=floor((dist/GROUPVEL_MIN(iband)+LONG_P(iband)/2.-t0_inp)/dt_inp)!floor((dist/2-t0_inp)/dt_inp)
          win_b=max(win_b,1)
          ...
          !!**************** 2. window ******************************************
          glob_num_win(irec,icomp)=1
          tstart=dist/GROUPVEL_MAX(iband)-LONG_P(iband)/2.!BinHe modified advised by Kai
          tend=dist/GROUPVEL_MIN(iband)+LONG_P(iband)/2.  !BinHe modified advised by Kai
          ...
          call measure_adj_fwat(dat_inp_bp,syn_inp_bp,tstart,tend,t0_inp,dt_inp,NDIM_CUT,net,sta,&
                                ...
                                T_pmax_dat,T_pmax_syn,adj_syn_all,file_prefix0,out_imeas,bandname)
          glob_file_prefix0(irec)=trim(file_prefix0)
          glob_net(irec)=trim(network_name(irec)) !trim(net)
          glob_sta(irec)=trim(station_name(irec)) !trim(sta)
          glob_chan_dat(irec,icomp)=trim(CH_CODE)//trim(RCOMPS(icomp)) !trim(chan_dat)
          glob_tstart(irec,icomp,1)=tstart
          glob_tend(irec,icomp,1)=tend
          glob_window_chi(irec,icomp,1:NCHI,1)=window_chi(1:NCHI)
...
subroutine meas_adj_leq(icmt, ievt, irec, glob_sem_disp,iband, fstart0, fend0, bandname, &
    ...
    do icomp=1,NRCOMP
      ...
      if ( findfile ) then
       ...
       ! data are processed with procedures in the order of those shown
       ! in seis_process/process_data.pl
       dt_inp=DT !!!! Here, I set dt=0.01 to interpolate data and syn
       t0_inp=-t0
       t1_inp=-t0+(NSTEP-1)*DT
       NDIM_CUT=(t1_inp-t0_inp)/dt_inp+1
       dat_inp(:)=0.
       ...
       if (CMT3D_INV) then  
         if (icmt>0) then
           dummystring=trim(OUTPUT_FILES)//'/'//trim(network_name(irec))//'.'&
           //trim(station_name(irec))//'.'//trim(CH_CODE)//trim(RCOMPS(icomp))&
           //'.syn.sac'//'.'//trim(bandname)//'.'//trim(CMT_PAR(icmt)) 
           call dwsac0(trim(dummystring),syn_inp_bp,NDIM_CUT,t0_inp,dt_inp,knetwk,kstnm, &
                    kcmpnm,dist,az,baz,dble(stla),dble(stlo),dble(evla),dble(evlo),dble(evdp))
         else
           dummystring=trim(OUTPUT_FILES)//'/'//trim(network_name(irec))//'.'&
...
subroutine sum_adj_source(irec, simu_type, adj_syn_all_sum, baz_all, stf_array, avgamp)

    character(len=MAX_STRING_LEN)                    :: adjfile,bandname,datafile,simu_type
    integer                                          :: icomp, iband, npt1, num_adj, irec, ier
    ...
    do icomp=1,NRCOMP
        ...
        do iband=1,NUM_FILTER
            ...
            if ( findfile ) then
                ...
                if(ADJ_SRC_NORM.and.simu_type=="noise".and.maxval(abs(datarray)).ne.0) then
