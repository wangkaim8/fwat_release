...
subroutine model_update_opt()
  ...
  use fullwave_adjoint_tomo_par, only: OPT_METHOD
 ...
 ! use tomography_model_tiso
  ...
  if (myrank == 0) then
    print *
    print *,'***********'
    print *,'program add_mode_iso_'//trim(OPT_METHOD)//': '
    print *,'  NPROC: ',NPROC
    print *
  ...
  if (trim(OPT_METHOD)=='CG'.and. iter_current>0) then
     call read_kernels_cg_iso_old()
  endif

  ! calculates gradient
  ! steepest descent method
  if (trim(OPT_METHOD)=='SD' .or. iter_current==0) then
     call get_sd_direction_iso()
  elseif (trim(OPT_METHOD)=='CG') then 
