...
subroutine sum_preconditioned_kernels_fwat(ekernel_dir_list,output_dir)
...
! on all GLL points, which are indexed (i,j,k,ispec)
  ...
  use tomography_par
  use fullwave_adjoint_tomo_par, only: PRECOND_TYPE, OUT_FWAT_LOG
...
subroutine smooth_sem_fwat(sigma_h,sigma_v,kernel_names_comma_delimited,input_dir,&
  ...
  use taper3d
  use fullwave_adjoint_tomo_par, only: KERNEL_TAPER
  ...
  do inum = 1,num_slices
    ...
    !if (myrank == 0) print *, 'NSPEC_N=',NSPEC_N
    !if (myrank == 0) print *, 'NGLOB_N=',NGLOB_N
    !if (myrank == 0) print *, 'NSPEC_IRREGULAR_N=',NSPEC_IRREGULAR_N
...
!---------------------------------------------------
subroutine smooth_sem_sph_pde_fwat(sigma_h,sigma_v,kernel_name,input_dir,&
                      output_dir,USE_GPU)
  ...
  if (myrank == 0) print *,"Running XSMOOTH_SEM_SPH_PDE"
  call synchronize_all()
  call cpu_time(t1)
...
end subroutine smooth_sem_sph_pde_fwat

! *************************************************
! code provided by Tianshi, 2022.11.14
subroutine smooth_sem_pde_fwat(sigma_h,sigma_v,kernel_name,input_dir,output_dir,USE_GPU)
  use constants, only: HUGEVAL
  use postprocess_par, only: CUSTOM_REAL,NGLLX,NGLLY,NGLLZ,NDIM,NGLLSQUARE, &
    MAX_STRING_LEN,IIN,IOUT,GAUSSALPHA,GAUSSBETA,PI,TWO_PI
  use specfem_par
  ...
  real(kind=CUSTOM_REAL), dimension(:), allocatable :: dat_glob, ddat_glob
  real(kind=CUSTOM_REAL), dimension(:), allocatable :: rvol
  real(kind=CUSTOM_REAL) :: min_rvol ! minimun of rvol
  real(kind=CUSTOM_REAL), dimension(:,:,:,:),allocatable :: rvol_local
  integer :: i,j,k,l,iglob,ier,ispec,ispec_p,iphase,ispec_irreg
  ...
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ) :: temp_xix,temp_zero
  ...
  !read(IIN) NGLOB_N
  !read(IIN) NSPEC_IRREGULAR_N!BinHe
  if (myrank == 0) print *, 'NSPEC_N=',NSPEC_AB
  if (myrank == 0) print *, 'NGLOB_N=',NGLOB_AB
  if (myrank == 0) print *, 'NSPEC_IRREGULAR_N=',NSPEC_IRREGULAR
  !close(IIN)

  ! allocates mesh arrays
  !allocate(dummy(NGLLX,NGLLY,NGLLZ,NSPEC_N),stat=ier)
  !if (ier /= 0) call exit_MPI_without_rank('error allocating array dummy')
  !allocate(irregular_element_number(NSPEC_N),stat=ier)
  !if (ier /= 0) call exit_MPI_without_rank('error allocating array irregular_element_number')
  ...
  call check_mesh_distances(myrank,NSPEC_AB,NGLOB_AB,ibool,xstore,ystore,zstore,&
                            ...
                            distance_min_glob,distance_max_glob)
  call bcast_all_singlecr(distance_min_glob)
  if (myrank == 0) print *, 'distance_min_glob = ', distance_min_glob

  !! determine ch, cv, ntstep
  cmax = distance_min_glob ** 2 / 6.0
  if (myrank == 0) print *, 'cmax=', cmax
  if (sigma_v >= sigma_h) then
    cv = cmax
    ch = cv * (sigma_h ** 2) / (sigma_v ** 2)
  else
  ...
  call define_derivation_matrices(xigll,yigll,zigll,wxgll,wygll,wzgll, &
                                  ...
                                  hprimewgll_xx,hprimewgll_yy,hprimewgll_zz, &
                                  wgllwgll_xy,wgllwgll_xz,wgllwgll_yz)
  if (myrank == 0) print *, 'define derivation matrice complete.'
  ...
  if (myrank == 0) print *, 'allocating data matrix:'
  allocate(dat(NGLLX,NGLLY,NGLLZ,NSPEC_AB))
  allocate(dat_glob(NGLOB_AB))
  allocate(ddat_glob(NGLOB_AB))
  allocate(buffer_send_vector_ext_mesh_smooth( &
  ...
  ! prepare assemble array
  if (myrank == 0) print *, 'preparing assemble array:'
  allocate(rvol(NGLOB_AB))
  rvol(:) = 0.0
  allocate(rvol_local(NGLLX,NGLLY,NGLLZ,NSPEC_AB))
  do ispec = 1, NSPEC_AB
    ispec_irreg = irregular_element_number(ispec)
    !if (myrank == 0) print *, 'ispec_irreg=', ispec_irreg
    do k=1,NGLLZ;do j=1,NGLLY;do i=1,NGLLX
      weight =  wxgll(i)*wygll(j)*wzgll(k)
      !if (myrank == 0) print *,'x:',i,'y:',j,'z:',k,'wxgll:',wxgll,'weight:',weight
      if (ispec_irreg /= 0) then
          jacobianl = jacobian(i,j,k,ispec_irreg)
      else
          jacobianl = jacobian_regular
      endif
      !if (myrank == 0) print *, 'jacobianl=', jacobianl
      rvol_local(i,j,k,ispec) = real(dble(jacobianl)*weight,kind=CUSTOM_REAL)
      iglob = ibool(i,j,k,ispec)
      rvol(iglob) = rvol(iglob) + rvol_local(i,j,k,ispec)
    enddo;enddo;enddo
  ...
  call assemble_MPI_scalar_blocking(NPROC,NGLOB_AB,rvol, &
                       ...
                       nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                       my_neighbors_ext_mesh)
  min_rvol=minval(rvol)
  if (myrank == 0) print *, 'min_rvol=', min_rvol
  rvol(:) = 1.0 / rvol(:)

  ! read in data to be smoothed
  ! data file
  if (myrank == 0) print *, 'reading data file: '
  write(prname,'(a,i6.6,a)') trim(input_dir)//'/proc',myrank,'_'
  local_data_file = trim(prname) // trim(kernel_name) // '.bin'
  open(unit = IIN,file = trim(local_data_file),status='old',action='read',&
       form ='unformatted',iostat=ier)
  ...
  do ispec = 1, NSPEC_AB
    ...
    enddo;enddo;enddo
  enddo
  ! temp array for regular mesh
  temp_xix=xix_regular
  temp_zero=0.0
  ...
  do istep = 1, ntstep
    ...
    do iphase = 1,2
      ...
      do ispec_p = 1,num_elements
        ...
        ispec_irreg = irregular_element_number(ispec)
        if (ispec_irreg /= 0) then
          call get_gradient_element(dat(:,:,:,ispec_irreg), &
            dx_elem,dy_elem,dz_elem,&
            xix(:,:,:,ispec_irreg),xiy(:,:,:,ispec_irreg),xiz(:,:,:,ispec_irreg),&
            etax(:,:,:,ispec_irreg),etay(:,:,:,ispec_irreg),etaz(:,:,:,ispec_irreg),&
            gammax(:,:,:,ispec_irreg),gammay(:,:,:,ispec_irreg),gammaz(:,:,:,ispec_irreg))
          !if (myrank == 0) print *,"ispec=",ispec,", ispec_irreg=",ispec_irreg
          !if (myrank == 0) print *,etay(:,:,:,ispec_irreg)
        else
          call get_gradient_element(dat(:,:,:,ispec), &
            dx_elem,dy_elem,dz_elem,&
            temp_xix,temp_zero,temp_zero,&
            temp_zero,temp_xix,temp_zero,&
            temp_zero,temp_zero,temp_xix)
          !if (myrank == 0) print *,"ispec=",ispec,", ispec_irreg=",ispec_irreg
          !if (myrank == 0) print *,temp_zero
        endif
        do k=1,NGLLZ;do j=1,NGLLY;do i=1,NGLLX
          if (ispec_irreg /= 0) then
            xixl = xix(i,j,k,ispec_irreg)
          ...
          !if (myrank == 0) print *,"ispec=",ispec,", ispec_irreg=",ispec_irreg, "jacobianl=",jacobianl
  ...
  if (myrank == 0) print *,'free memory after smooth. '
  !deallocate(irregular_element_number)
  !deallocate(dummy)
  deallocate(dat, dat_glob, ddat_glob)
  deallocate(buffer_send_vector_ext_mesh_smooth, &
