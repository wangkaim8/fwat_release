...
subroutine collect_chi(glob_file_prefix0, glob_net, glob_sta,  glob_chan_dat, glob_tstart, &
    ...
    double precision, dimension(nrec,NRCOMP,FLEX_NWIN)  :: glob_tstart
    double precision, dimension(nrec,NRCOMP,FLEX_NWIN)  :: glob_tend
    double precision, dimension(nrec,NRCOMP,NCHI,FLEX_NWIN)  :: glob_window_chi
    double precision, dimension(nrec,NRCOMP,FLEX_NWIN)  :: glob_tr_chi
    double precision, dimension(nrec,NRCOMP,FLEX_NWIN)  :: glob_am_chi
    double precision, dimension(nrec,NRCOMP,FLEX_NWIN)  :: glob_T_pmax_dat
    double precision, dimension(nrec,NRCOMP,FLEX_NWIN)  :: glob_T_pmax_syn
    integer         , dimension(nrec,NRCOMP,FLEX_NWIN)  :: glob_imeas
    ...
    integer :: nrec_local_max, my_nrec_local,iwin
        ...
        if (myrank == 0) then
           ...
           if (nsta_irank > 0) then
              ...
              allocate(tstart_loc(nsta_irank,NRCOMP,FLEX_NWIN))
              allocate(tend_loc(nsta_irank,NRCOMP,FLEX_NWIN))
              allocate(window_chi_loc(nsta_irank,NRCOMP,NCHI,FLEX_NWIN))
              allocate(tr_chi_loc(nsta_irank,NRCOMP,FLEX_NWIN))
              allocate(am_chi_loc(nsta_irank,NRCOMP,FLEX_NWIN))
              allocate(T_pmax_dat_loc(nsta_irank,NRCOMP,FLEX_NWIN))
              allocate(T_pmax_syn_loc(nsta_irank,NRCOMP,FLEX_NWIN))
              allocate(imeas_loc(nsta_irank,NRCOMP,FLEX_NWIN))
              !! data to receive
              irec_local=0
              tag   = 2001 !MPI_ANY_TAG
              ...
              call MPI_RECV(tstart_loc, nsta_irank*NRCOMP*FLEX_NWIN, MPI_REAL, irank, tag, my_local_mpi_comm_world, status,  ier)
              call MPI_RECV(tend_loc, nsta_irank*NRCOMP*FLEX_NWIN, MPI_REAL, irank, tag, my_local_mpi_comm_world, status,  ier)
              call MPI_RECV(window_chi_loc, nsta_irank*NRCOMP*NCHI*FLEX_NWIN, MPI_REAL, irank, tag, my_local_mpi_comm_world, &
                           status,  ier)
              call MPI_RECV(tr_chi_loc, nsta_irank*NRCOMP*FLEX_NWIN, MPI_REAL, irank, tag, my_local_mpi_comm_world, status,  ier)
              call MPI_RECV(am_chi_loc, nsta_irank*NRCOMP*FLEX_NWIN, MPI_REAL, irank, tag, my_local_mpi_comm_world, status,  ier)
              call MPI_RECV(T_pmax_dat_loc, nsta_irank*NRCOMP*FLEX_NWIN, MPI_REAL, irank, tag, my_local_mpi_comm_world, & 
                           status,  ier)
              call MPI_RECV(T_pmax_syn_loc, nsta_irank*NRCOMP*FLEX_NWIN, MPI_REAL, irank, tag, my_local_mpi_comm_world, &
                           status,  ier)
              call MPI_RECV(imeas_loc, nsta_irank*NRCOMP*FLEX_NWIN, MPI_INTEGER, irank, tag, my_local_mpi_comm_world, status,  ier)
              do icomp=1,NRCOMP
                do irec_local = 1, nsta_irank
                  do iwin=1,FLEX_NWIN
                    irec=irank*nrec_local_max+irec_local
           ...
           if (myrank == irank .and. my_nrec_local > 0) then
              ...
              allocate(tstart_loc(my_nrec_local,NRCOMP,FLEX_NWIN))
              allocate(tend_loc(my_nrec_local,NRCOMP,FLEX_NWIN))
              allocate(window_chi_loc(my_nrec_local,NRCOMP,NCHI,FLEX_NWIN))
              allocate(tr_chi_loc(my_nrec_local,NRCOMP,FLEX_NWIN))
              allocate(am_chi_loc(my_nrec_local,NRCOMP,FLEX_NWIN))
              allocate(T_pmax_dat_loc(my_nrec_local,NRCOMP,FLEX_NWIN))
              allocate(T_pmax_syn_loc(my_nrec_local,NRCOMP,FLEX_NWIN))
              allocate(imeas_loc(my_nrec_local,NRCOMP,FLEX_NWIN))
              ...
              call MPI_SEND(tstart_loc,  my_nrec_local*NRCOMP*FLEX_NWIN, MPI_REAL, 0, tag, my_local_mpi_comm_world, ier)
              call MPI_SEND(tend_loc,  my_nrec_local*NRCOMP*FLEX_NWIN, MPI_REAL, 0, tag, my_local_mpi_comm_world, ier)
              call MPI_SEND(window_chi_loc,  my_nrec_local*NRCOMP*NCHI*FLEX_NWIN, MPI_REAL, 0, tag, my_local_mpi_comm_world, ier)
              call MPI_SEND(tr_chi_loc,  my_nrec_local*NRCOMP*FLEX_NWIN, MPI_REAL, 0, tag, my_local_mpi_comm_world, ier)
              call MPI_SEND(am_chi_loc,  my_nrec_local*NRCOMP*FLEX_NWIN, MPI_REAL, 0, tag, my_local_mpi_comm_world, ier)
              call MPI_SEND(T_pmax_dat_loc,  my_nrec_local*NRCOMP*FLEX_NWIN, MPI_REAL, 0, tag, my_local_mpi_comm_world, ier)
              call MPI_SEND(T_pmax_syn_loc,  my_nrec_local*NRCOMP*FLEX_NWIN, MPI_REAL, 0, tag, my_local_mpi_comm_world, ier)
              call MPI_SEND(imeas_loc, my_nrec_local*NRCOMP*FLEX_NWIN, MPI_INTEGER, 0, tag, my_local_mpi_comm_world, ier)
              deallocate(file_prefix0_loc)
              deallocate(net_loc)
              deallocate(sta_loc)
              deallocate(chan_dat_loc)
